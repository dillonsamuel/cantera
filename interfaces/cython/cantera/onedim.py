...
class FlameBase(Sim1D):
    ...
    def strain_rate(self):
        ...
        return self.flame.strain_rate

    @strain_rate.setter
    def strain_rate(self, strain):
        """ Set/get imposed strain rate """
        self.flame.strain_rate = strain
...
class CounterflowStrainImposed(FlameBase):
    """ A counterflow strain-imposed diffusion flame """
    __slots__ = ('fuel_inlet', 'flame', 'oxidizer_inlet')
    ...
    def set_initial_guess(self, data=None, group=None):
        ...
        # initialise U with hyperbolic tangent
        xx = np.linspace(zz[0], zz[-1], len(self.flame.grid))
        spread_rate = 0.5 * (U0f - U0o) * (1 + np.tanh(xx/(dz))) + U0o

        self.set_profile('velocity', [0.0,0.5, 1.0], [u0f,0.0, -u0o])
        self.set_profile('spread_rate', zrel, spread_rate[::-1])
        self.set_profile("lambda", [0.0, 1.0], [1.0, 1.0])
        self.set_profile('T', zrel, T)
